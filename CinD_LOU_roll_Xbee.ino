...
int    rad;   // the radius
int    num;   // the number
int    valL = 0;   // the Left motor command value
int    valR = 0;   // the Right motor command value
SoftwareSerial lcdSerial(3,2); // pin 2 = TX, pin 3 = RX (unused)
SoftwareSerial xbeeSerial(5,4);//pin 4 = TX, pin 5 = RX (unused)
...
void setup() {
  ...
  xbeeSerial.begin(9600);
    xbeeSerial.write(valL);
    xbeeSerial.write(valR);    
}
